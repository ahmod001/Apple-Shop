...
<div class="main_content">
    ...
    <script>
        ...
        function updateProductDetails(data) {
            ...
            productImg.src = data.img1;
            productImg.dataset.zoomImage = data.img1;

            // Update gallery images
            const galleryItems = document.querySelectorAll('#pr_item_gallery .item');
            galleryItems.forEach((item, index) => {
                const galleryImage = item.querySelector('.product_gallery_item');
                galleryImage.href = data[`img${index + 1}`];
                galleryImage.dataset.image = data[`img${index + 1}`];
                galleryImage.dataset.zoomImage = data[`img${index + 1}`];
                const smallImage = galleryImage.querySelector('img');
                smallImage.src = data[`img${index + 1}`];
            });

            // Update product title and description
            const productTitle = document.querySelector('.product_title a');
            productTitle.textContent = data.product.title;

            const productDescription = document.querySelector('.pr_desc p');
            productDescription.textContent = data.description;
        }

        // `/product-detail/${productId}`
        async function getProductDetails() {
            const res = await axios.get(`/product-detail/1`)
            await updateProductDetails(res.data?.data);
        }()
    </script>
